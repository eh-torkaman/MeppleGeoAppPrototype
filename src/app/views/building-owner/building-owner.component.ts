...
@Component({
  ...
  styleUrls: ['./building-owner.component.scss'],
})
export class BuildingOwnerComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.Buildings$.subscribe((rs) => {
      ...
      rs.features.map((it) => it.properties.OWNER);

      let recuded = rs.features.reduce((pv, cv, cix) => {
        if (pv[cv.properties.OWNER]) pv[cv.properties.OWNER]++;
        else pv[cv.properties.OWNER] = 1;

        return pv;
      }, {} as { [OWNERName: string]: number });
